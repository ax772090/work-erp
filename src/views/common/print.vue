...
export default {
  ...
  methods: {
    ...
    storageDetailData (id) {
      this.$http
        .get(this.$http.adornUrl(`/warehouse/whinstockpo/billprint/${id}`))
        .then(({ data }) => {
          if (data && data.code === 0) {
            this.$set(data.poInstockDto, 'addUserName', window.sessionStorage.getItem('addUserName'))
            ...
            this.dataForm.printTime = data.poInstockDto.dateTime
          }
        })
    },
    // // 初始化
