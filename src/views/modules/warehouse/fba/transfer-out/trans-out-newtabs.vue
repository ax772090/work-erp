<template>
  ...
  >
    <el-tabs v-model="activeName" type="border-card" @tab-click="elTabsTabClick">
      <el-tab-pane :name="tabs[0].tabname" lazy>
        <span slot="label">
          <i class="el-icon-date"></i>库存调出
        </span>
        <transfer-out-detail
          v-model="dataForm.transferOutDetail"
          :ref="tabs[0].refname"
          @validate="validateObj"
          v-if="visible"
        ></transfer-out-detail>
      </el-tab-pane>
      <el-tab-pane :name="tabs[1].tabname" lazy :disabled="packingDisabled">
        <span slot="label">
          <i class="el-icon-date"></i>装箱清单
        </span>
        <packing-detail v-model="dataForm.packingDetail" :ref="tabs[1].refname" v-if="visible"></packing-detail>
      </el-tab-pane>
    </el-tabs>
    <span slot="footer" class="dialog-footer">
      <el-button @click="cancel('dataForm')"><i class="iconfont erp-icon-quxiao"></i>取消</el-button>
      <el-button
        v-if="(activeName == 'transferOutDetail') && isShowBtn"
        type="primary"
        @click="nextStepValidate()"
        :disabled="nextStepDisabled"
      >下一步</el-button>
      <el-button
        v-if="(activeName == 'packingDetail') && isShowBtn"
        type="primary"
        @click="preStep()"
      >上一步</el-button>
      <el-button
        v-if="(activeName == 'packingDetail') && !isDisabled"
        type="primary"
        @click="dataFormSave()"
      ><i class="iconfont erp-icon-baocun"></i>保存</el-button>
      <el-button
        v-if="(activeName == 'packingDetail') && !isDisabled"
        type="primary"
        @click="dataFormSubmit()"
      ><i class="iconfont erp-icon-queding"></i>确认调出</el-button>
    </span>
  </el-dialog>
</template>
