...
export default {
  ...
  mounted () {
    Bus.$on('MainNavbar-MainContent', (msg) => {
      // this.tabsCloseOtherHandle()
      this.tabsCloseAllHandle()
      window.location.reload()
    })
    Bus.$on('information-center', (msg) => {
      this.infoDataHandle()
    })
  },
  activated () {
    console.log('activated触发了1')
  },
  deactivated () {
    console.log('deactivated触发了2')
  },
  methods: {
    // 消息中心触发
    infoDataHandle (tabName) {
      let tab
      tab = this.mainTabs.filter(item => item.name === tabName)
      if (tab.length >= 1) {
