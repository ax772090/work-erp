  ...
  <el-dialog title="打印其他出库"
    ...
    <div ref="prints"
      ...
      <div class="print-header">
        ...
        <div>
          <div class="spanchp">&nbsp;&nbsp;其他出库单&nbsp;&nbsp;</div>
        </div>
      </div>
      <el-row class="print-center">
        <el-col :span="8">
          <div>出库类型：{{printdata.dictOutstockTypeName}}</div>
          <div>出库日期：{{printdata.outstockDate}}</div>
          <div>出库原因：{{printdata.reason}}</div>
        </el-col>
        <el-col :span="8">
        ...
        <el-col :span="8">
          <div>出库单号：{{printdata.code}}</div>
          <div>单据状态：{{printdata.dictDocStatusName}}</div>
          <div>部门：{{printdata.deptName}}</div>
        </el-col>
      </el-row>
      <table border="1"
             class="gridtable">
        ...
        <tr v-for="(item, index) in printdata.whOutstockDetail"
            :key="index">
          <td>{{index+1}}</td>
          <td>{{item.prodCode}}</td>
          <td>{{item.prodName}}</td>
...
export default {
  ...
  methods: {
    init (printdata) {
      this.visible = true;
      this.printdata = printdata.WhOutstockDto;
      this.$nextTick(() => {
        this.getDataList();
      });
    },
