  ...
  <el-dialog title="海外仓费用维护"
    ...
    <el-form :model="dataForm"
      ...
      <el-form-item label="SellerSKU"
                    prop="sellersku">
        <select-seach v-model="dataForm.sellersku"
                      httpUrl="/prod/prodchannelsku/queryForComplete"
                      httpMethod="get"
                      requestParamsKey="code"
                      data-label="sellersku"
...
export default {
  ...
  methods: {
      ...
      if (this.dataForm.id) {
        this.$http({
          url: this.$http.adornUrl(
            `fin/platformfee/info/${this.dataForm.id}/overseas`
          ),
          method: 'get'
        }).then(({ data }) => {
          if (data && data.code === 0) {
            this.dataForm = Object.assign({}, data.mwsoverseasFee, { updUserName: null, updTime: null, updUser: null })
          } else {
            this.$notify.error({
              title: '失败',
              message: data.msg,
    ...
    prodCodeChange (item) {
      ...
      // })
      // this.$http
      //   .get(this.$http.adornUrl('prod/prodchannelsku/queryForComplete'), { params: { code: val.id } }).then(({
      //     data
      //   }) => {
        ...
        this.$refs['dataForm'].validate(valid => {
          if (valid) {
            this.$http({
              ...
              data: {
                feeType: 'overseas',
                mwsOverseasFeeEntity: this.dataForm
              }
            }).then(({ data }) => {
              if (data && data.code === 0) {
