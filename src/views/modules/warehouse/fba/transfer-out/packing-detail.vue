        ...
        <el-col :span="6">
          <el-form-item label="FBAShipmentID"
            ...
            <select-all v-model="dataForm.fbaShipmentId"
                        ...
                        data-value="key"
                        data-label="value"
                        placeholder="选择FBAShipmentId"></select-all>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
...
export default {
  ...
  data () {
    return {
      fbaShipmentIdOptions: [], // shipemntId的list
  ...
  methods: {
        ...
        if (this.dataForm.id) {
          this.$http({
            ...
            if (data && data.code === 0) {
              ...
              this.dataForm.packingDetailsList =
                data.transferPackingInfo.whPackingDetailList
            } else {
              // this.$message.error(data.msg)
            }
          })
        }
      } else if (id && name === 'delivery') {
        // FBAShipment下来的新增，id为shipmentId
        this.initDataForm(dataForm)
        // this.dataForm.fbaShipmentId = id;
        this.dataForm.id = ''
    ...
    initDataForm (dataForm) {
      ...
      let channelId = dataForm.transferOutDetail.channelId
      arr.forEach(element => {
      ...
      // 通过箱数生成装箱清单
      this.initPackingDetailList(arr, channelId)
    },
    initList (id) {
      this.$nextTick(() => {
        if (this.dataForm.id) {
          this.$http({
            ...
            if (data && data.code === 0) {
              this.dataForm = Object.assign(
                {},
                this.dataForm,
                data.transferPackingInfo.whPackingEntity
              )
    ...
    initPackingDetailList (transferOutDetailsList, channelId) {
      // this.$nextTick(() => {

      this.$http({
        url: this.$http.adornUrl(`warehouse/whtransferout/packing-detail`),
        method: 'post',
        // data:transferOutDetailsList
        data: {
          channelId: channelId,
          dto: transferOutDetailsList
        }
      }).then(({ data }) => {
        if (data && data.code === 0) {
          ...
          if (data.shipments) {
            this.fbaShipmentIdOptions = data.shipments
          } else {
            this.fbaShipmentIdOptions = []
          }
        } else {
          this.$notify.error({
            title: '错误',
